...
export function AuthProvider({ children }) { 
  ...
  const finalizeLogin = async (token, userId, userEmail) => {
    api.defaults.headers.common['Authorization'] = `Bearer ${token}`; 
    localStorage.setItem('nexora_token', token);

    let role = 'admin'; // default for demo 
    let plan = 'pro'; 
    let name = userEmail;
  ...
  const login = async (email, password) => { 
    ...
    return await finalizeLogin(token, userId, userEmail);
  };
  ...
  return ( 
    <AuthContext.Provider value={{ user, login, verifyMfaLogin, register, logout, loading }}> 
      {children} 
